...
class DCGAN(object):
    def __init__(self, sess, input_height=108, input_width=108, crop=True,
        ...
        if self.dataset_name == 'mnist':
            ...
            if len(imreadImg.shape) >= 3: #check if image is a non-grayscale image by checking channel number(grayscale has 1, color has at least three)
    ...
    def discriminator(self, image, y=None, reuse=False, batch_size = None):
        ...
        with tf.variable_scope("discriminator") as scope: #validate if value is discriminator
            if reuse:
    ...
    def get_test_data(self):
        ...
        batch = []
        for name in self.test_data_names:

            if 'png' in name:
               batch.append(get_image(name, input_height=self.input_height, input_width=self.input_width,
    ...
    def train_anomaly_detector(self, config, test_data, test_data_name):
        ...
        fp = open('anomaly_score_record2222.txt','a+')
        for epoch in range(config.test_epoch):
            ...
            #ano_score, res_loss = self.sess.run([z_optim, self.anomaly_score, self.res_loss],   feed_dict=feed_dict)
            ...
            if epoch in save_epoch:
                ...
                print (str(epoch)+ ' save_ ' + str(samples.shape))
            ...
            if not self.grayscale:
                ...
                try:
                   print('saving image : ' + str(samples.shape) + ',' + filename[0] + ',' +path)
                ...
                try:
                   scipy.misc.imsave(os.path.join(path,filename[1]),errors)
                    ...
                    print('error(save)')
                    scipy.misc.imsave(os.path.join(path,filename[1]),errors[0])

                np.save('./{}/test_error_{}_{:02d}.png'.format(config.test_dir, test_data_name, epoch), errors)
        #fp.write(str(epoch) + ',' + str(ano_score) + '\n')
        #fp.write(filename[0]+ ',' + str(ano_score) + ',' +path+'\n')


        fp.write(str(ano_score)+'\n')
        fp.close()
